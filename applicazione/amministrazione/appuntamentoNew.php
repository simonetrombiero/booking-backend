...
unset($_SESSION['arrayTrattamenti']);


$data = getRequest("data");
$ore = getRequest("ore");
$idPostazione = getRequest("id");
$idClienteTMP = getRequest("idCliente");
$nominativoTMP='';
if(isset($idClienteTMP)){
    $clienteTMP = DAOFactory::getClientiDAO()->load($idClienteTMP);
    if(!isBlankOrNull($clienteTMP->nome)){
        $nominativoTMP = $clienteTMP->nome." ".$clienteTMP->cognome;
    }
        ...
        <div class="col-1-1">
            ...
            <div>
                <form action="index.php" method="post" id="formAppuntamento" name="formAppuntamento">
                    ...
                    ?>"/>
                    <div class="slider"><span class="titleTasti">Cerca Cliente:</span>&nbsp;<input id="cliente" name="cliente" type="text" style="width:52%; height:30px; border:1px solid #CCC; border-radius: 5px; padding-left:5px; padding-right:5px;" <?php if($nominativoTMP!=''){echo "value='$nominativoTMP'";}?>>&nbsp;<input type="image" src="img/icone/view.png" width="30">&nbsp;<a href="javascript: nuovoCliente()" class="btn btn-green" ><img src="img/icone/aggiungi.png" height="20"> Aggiungi Cliente</a>
                    ...
                    <div style="width: 100%; border: 1px solid #CCC; border-radius: 10px; padding: 10px; display: none;" id="nuovoCliente">
                        ...
                        <input type="hidden" id="idCliente" name="idCliente" <?php if($nominativoTMP!=''){echo "value='$idClienteTMP'";}?>>
...
<!--<span class="titleTasti">Cerca Cliente:</span>&nbsp;<input type="text" style="width:80%; height:30px; border:1px solid #CCC; border-radius: 5px; padding-left:5px; padding-right:5px;">&nbsp;<input type="image" src="img/icone/view.png" width="30">-->
                    ...
                    <div style="width: 100%; border: 1px solid #CCC; border-radius: 10px; padding: 10px;">
                        <div>
                            ...
                            <?php
                            //$status = DAOFactory::getPrenotazioneStatusDAO()->queryAll(); 
                            $status = DAOFactory::getPianoTrattamentoStatusDAO()->queryAll();
                            ?>
                            ...
                            <select name="statusAppuntamento" id="statusAppuntamento" style="width: 45%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204);" required="" >
                                ...
                                for ($i = 0; $i < count($status); $i++) {
                                    ...
                                    ?>><?php echo $stato->descrizione; ?></option>
                                        <?php } ?>
                            </select> 
                    ...
                    <div>
                        ...
                        <table style="width: 100%" id="datatable">
                            <thead>
                                <tr>
                                    <th align="left" style="width: 5%;">N</th>
                                    <th align="left">Descrizione</th>



                                    <th align="left">Data e Ora</th>
                                    <th align="left">Ripetizione</th>
                                    <th align="left">Postazione e Operatore</th>
                            ...
                            <tbody>
                                ...
                                for ($i = 0; $i < count($listaTrattamenti); $i++) {
                                    ...
                                    <tr <?php echo $backgroundTR; ?>>
                                        <td style="border-bottom: 1px dotted #057fd0;"><input name="nSedute" id="nSedute<?php echo $i; ?>" type="text" style="width: 100%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204); min-width: 30px;"></td>
                                        <td style="border-bottom: 1px dotted #057fd0;"><input type="hidden" name="idTrattamento" id="idTrattamentoAss<?php echo $i; ?>" value="<?php echo $trattamento->id; ?>">
                                            <a href="javascript: prenotazioneAggiungiTrattamento('<?php echo $trattamento->id; ?>', '<?php echo $i; ?>')" style="font-weight: bold">   
                                                <?php
                                        ...
                                        <td style="border-bottom: 1px dotted #057fd0;">
                                            <input type="text" name="dataAppuntamento" class="dataAppuntamento" id="dataAppuntamento<?php echo $i; ?>" value="<?php echo $data; //if(isset($appuntamento->data)){ echo dateFromDb($appuntamento->dataNascita); }      ?>" style="width: 90%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204); min-width: 60px;" placeholder="Data Appuntamento" />&nbsp;<input type="text" name="OraInizioAppuntamento" id="OraInizioAppuntamento<?php echo $i; ?>" value="<?php echo $ore; //if(isset($appuntamento)){ echo $appuntamento->inizio; }      ?>" style="width: 90%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204);" class="timepicker" placeholder="Ora Inizio"></td>
                                        <td style="border-bottom: 1px dotted #057fd0;">
                                            <input name="ripetizione" id="ripetizione<?php echo $i; ?>" type="number" value="1" min="1" style="width: 30%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204); min-width: 30px;">&nbsp;<select  style="width: 65%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204);" required="" onchange="setGiorniRipetizione(this.selectedIndex, <?php echo $i; ?>)" name="giorniRipetizione" id="giorniRipetizione<?php echo $i; ?>">
                                                <option value="g">giorno</option>
                                                <option value="s">settimana</option>
                                                <option value="m">mese</option>
                                                <option value="a">anno</option>
                                            </select><br>
                                            <div id="divSelezioneGiorno_<?php echo $i; ?>" style="display: none"><input type="checkbox">L<input type="checkbox">M<input type="checkbox">M<input type="checkbox">G<input type="checkbox">V</div>

                                        </td>
                                        <td style="border-bottom: 1px dotted #057fd0;">
                                            <select name="postazioneAppuntamento" id="postazioneAppuntamento<?php echo $i; ?>" style="width: 90%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204);" required="" >
                                                <option value="0"> Postazione </option>
                                                <?php
                                                for ($ik = 0; $ik < count($postazioni); $ik++) {
                                                    $postazione = $postazioni[$ik];
                                                    ?>
                                                    <option value="<?php echo $postazione->id ?>" <?php
                                                    if (isset($idPostazione)) {
                                                        if ($idPostazione == $postazione->id) {
                                                            echo "selected='selected'";
                                                        }
                                                    }
                                            ...
                                            <select name="operatoreAppuntamento" id="operatoreAppuntamento<?php echo $i; ?>" style="width: 90%; padding: 5px; border-radius: 5px; border: solid 1px rgb(204, 204, 204);" required="" >
...
//                            }
                                        ...
                                        <td style="vertical-align: middle; text-align: center; border-bottom: 1px dotted #057fd0;">
                                            <a href="javascript: prenotazioneAggiungiTrattamento('<?php echo $trattamento->id; ?>', '<?php echo $i; ?>')"> <img src='img/icone/aggiungi.png' height="20"> </a>
                    ...
                    <div style="clear:left;">&nbsp;</div>
                    <div style="width: 100%; border: 1px solid #CCC; border-radius: 10px; padding: 10px;">
                        <span style="font-weight: bolder;">Trattamenti Associati alla Prenotazione</span> <br><br>
...
//VERIFICA PRESENZA APPUNTAMENTO
        ...
        function setGiorniRipetizione(idSelezionato, posizione) {
            //alert("www");
            //alert(aaa);
            //alert(posizione)
            ...
            if (idSelezionato == '1') {
                //  alert("QUI");
                document.getElementById("divSelezioneGiorno_" + posizione).style.display = "block";
            } else {
                //alert("no");
                document.getElementById("divSelezioneGiorno_" + posizione).style.display = "none";
            }
