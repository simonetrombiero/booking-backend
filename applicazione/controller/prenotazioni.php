...
function salvaPrenotazioneM() {
    ...
    }else{ // inserisco un piano di trattamento fittizio
        $pianox = new PianoTrattamento();
        $pianox->idPaziente = $cliente;
        $pianox->titolo = "Prenotazione Singola";
        $pianox->data = date("Y-m-d");
        $pianox->seduteNumero = 1;
        $pianox->stato=2;
        $pianox->prenotazioni = 1;
        $pianox->isChiuso=1;
        
        $idPiano = DAOFactory::getPianoTrattamentoDAO()->insert($pianox);
    ...
    for ($jj = 0; $jj <= $nNuoviTrattamenti; $jj++) {
        ...
        if($nuovopiano){
            $dettPiano = new PianoTrattamentoItem();
            
            $dettPiano->idPiano = $idPiano;
            $dettPiano->idTrattamentoPiano = $trattamentiTMPint->id;
            $dettPiano->numeroRiga = $jj+1;
            $dettPiano->qta = 1;
            $dettPiano->trattamento = $trattamentiTMPint->trattamento;
            $dettPiano->prezzoUnitario = $trattamentiTMPint->costo;
            $dettPiano->totaleRiga = $trattamentiTMPint->costo;
            $dettPiano->status = 1;
            
            DAOFactory::getPianoTrattamentoItemDAO()->insert($dettPiano);
        }
    }
...
function salvaAppuntamenti() {
    ...
    $data = date("w", strtotime($dataAppuntamento));
    // exit;
    ...
    $dataSelezionataCompresa = getRequest("dataSelezionata");
    ...
    //variazione
    $arrayDateSedute = array();
    $kk = 0;
    $i = 0;

    switch ($giorniRipetizione) {
        case "g": //RIPETIZIONE GIORNALIERA

            while ($kk < $sedute) {

                $dataAppuntamentoTMP = date('Y-m-d', strtotime($dataAppuntamento . '+' . $i . ' day'));
                $gs = $data = date("w", strtotime($dataAppuntamentoTMP));
                if (($gs > 0) && ($gs < 6)) {
                    $arrayDateSedute[] = $dataAppuntamentoTMP;
                    $kk++;
                }
                $i++;
            }
            ...
            break;
        case "s": // RIPETIZIONE SETTIMANALE
            if ($dataSelezionataCompresa == 'si') {
                $arrayDateSedute[] = $dataAppuntamento;
                $i++;
                $kk++;
            }
            ...
            while ($kk < $sedute) {
                $dataAppuntamentoTMP = date('Y-m-d', strtotime($dataAppuntamento . '+' . $i . ' day'));
                $gs = $data = date("w", strtotime($dataAppuntamentoTMP));
                if (in_array($gs, $giornoSelezionato)) {
                    $arrayDateSedute[] = $dataAppuntamentoTMP;
                    $kk++;
                }
                $i++;
            ...
            break;
        case "m": // RIPETIZIONE MENSILE


            while ($kk < $sedute) {

                $dataAppuntamentoTMP = date('Y-m-d', strtotime($dataAppuntamento . '+' . $kk . ' months'));
                $gs = $data = date("w", strtotime($dataAppuntamentoTMP));
                if ($gs == 0) {
                    $dataAppuntamentoTMP = date('Y-m-d', strtotime($dataAppuntamentoTMP . '+1 day'));
                }
            ...
            break;
        case "a": //RIPETIZIONE ANNUALE

            while ($kk < $sedute) {

                $dataAppuntamentoTMP = date('Y-m-d', strtotime($dataAppuntamento . '+' . $kk . ' year'));
                $gs = $data = date("w", strtotime($dataAppuntamentoTMP));
                if ($gs == 0) {
                    $dataAppuntamentoTMP = date('Y-m-d', strtotime($dataAppuntamentoTMP . '+1 day'));
                }
...
//    print_r($arrayDateSedute);
//exit;
    // fine variazione



    for ($h = 0; $h < count($arrayDateSedute); $h++) {
        ...
        $prenotazioni->data = $arrayDateSedute[$h];
...
function salvaAppuntamento() {
    ...
    $statusAppuntamento = getRequest("statusAppuntamento");
    $dataAppuntamento = dateToDb(getRequest("dataAppuntamento"));
    $oraInizioAppuntamento = getRequest("oraInizioAppuntamento");
    $oraFineAppuntamento = getRequest("oraFineAppuntamento");
    $noteAppuntamento = getRequest("noteAppuntamento");

    $numeroTrattamenti = getRequest("numeroTrattamenti");

    $appuntamentoTMP = DAOFactory::getPrenotazioniDAO()->load($id);

    $appuntamentoTMP->status = $statusAppuntamento;
    $appuntamentoTMP->data = $dataAppuntamento;
    $appuntamentoTMP->oraInizio = $oraInizioAppuntamento;
    $appuntamentoTMP->oraFine = $oraFineAppuntamento;
    $appuntamentoTMP->note = $noteAppuntamento;

    DAOFactory::getPrenotazioniDAO()->update($appuntamentoTMP);

    for ($i = 0; $i < $numeroTrattamenti; $i++) {

        $idRiga = getRequest("idRiga" . $i);

        $operatore = getRequest("operatore" . $i);
        $postazione = getRequest("postazione" . $i);

        $rigaTMP = DAOFactory::getPrenotazioniDettaglioDAO()->load($idRiga);

        $rigaTMP->idOperatore = $operatore;
        $rigaTMP->idPostazione = $postazione;
        print_r($rigaTMP);
        DAOFactory::getPrenotazioniDettaglioDAO()->update($rigaTMP);
    }
...
function caricaListaPrenotazioni() {
    ...
    for ($i = 0; $i < count($eventi); $i++) {
        ...
        //print_r($prenotazioneMaster);
        $eventoTmp->prenotazione;
        ...
        $trattamento = $eventoTmp->trattamento;
//echo "xxx ". $eventoTmp->idOperatore."<br>";
        //$operatore = DAOFactory::getDipendenteDAO()->load($eventoTmp->idOperatore);
        $operatore = DAOFactory::getDipendenteDAO()->load($eventoTmp->idOperatore);
...
//funzioni per la stampa degli appuntamenti
function caricaInformazioniStampe() {
    $scelta = getRequest("tipoStampa");
    $giorno = getRequest("giorno");
    /*
     * VALORI AMMESSI
     * 
     *  "planner"    =>  Planner Giornaliero
     *  "operatori"  =>  Per Operatori
     *  "postazioni" =>  Per Postazioni
     */
    ...
    switch ($scelta) {
        case "operatori":
            ...
            include_once('vendors/appuntamenti.php');
        ...
        case "postazioni":
            ...
            for ($i = 0; $i < count($postazioni); $i++) {
                ...
                $appPs = DAOFactory::getPrenotazioniDAO()->queryPrenotazioniGiornoPostazioneOrderBYoraInizio($giorno, $postazione->id);
                if (count($appPs) > 0) {
                    $postazioniArray[] = $postazione;
                    $appuntamenti[] = $appPs;
                }
            }

            include_once('vendors/appuntamentiPos.php');


            break;

        default:
            //$orari = array("08:00", "09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00");
            $orari = array("09:00", "10:00", "11:00", "12:00", "15:00", "16:00", "17:00", "18:00", "19:00");
            //$orariFine = array("08:59", "09:59", "10:59", "11:59", "12:59", "13:59", "14:59", "15:59", "16:59", "17:59", "18:59", "19:59");
            $orariFine = array("09:59", "10:59", "11:59", "12:59", "15:59", "16:59", "17:59", "18:59", "19:59");
            $postazioni = DAOFactory::getPostazioniDAO()->queryAll();
            //$appuntamenti = array();
            //$postazioniArray = array();

            include_once('vendors/appuntamentiGen.php');
    ...
    //$nomeFile = "appuntamenti" . $giorno . ".pdf";
    $nomeFile = "appuntamenti.pdf";
    ...
    try {
        ...
        $html2pdf->Output("documentazione/" . $nomeFile, 'F');
        /*
          ...
          header("Content-Disposition: attachment;filename=documentazione/".$nomeFile);
          header("Content-Transfer-Encoding: binary "); */
        /* header("Cache-Control: public");
          header("Content-Description: File Transfer");
          header("Content-Disposition: attachment; filename=documentazione/appuntamenti.pdf");
          header("Content-Type: application/octet-stream");
          header("Content-Transfer-Encoding: binary");
          header('Cache-Control: must-revalidate');
          header('Content-Length: ' . filesize('documentazione/appuntamenti.pdf'));
          readfile('documentazione/appuntamenti.pdf'); */
